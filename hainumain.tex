% !Mode:: "TeX:UTF-8"
% \def\usewhat{dvipdfmx}                               % 定义编译方式 dvipdfmx 或者 pdflatex，默认为 dvipdfmx
                                                     % 方式编译，如果需要修改，只需改变花括号中的内容即可。
\documentclass[12pt,openany,oneside]{book}
% \usepackage{ctex}
% \documentclass{ctexbook}
                                                     % 本科生毕业论文通常采用单页排版
\input{setup/package}                                % 定义本文所使用宏包
\graphicspath{{figures/}}                            % 定义所有的 .eps 文件在 figures 子目录下
\begin{document}                                     % 开始全文
% \begin{CJK*}{UTF8}{song}                                % 开始中文字体使用
\input{setup/format}                                 % 完成对论文各个部分格式的设置
\frontmatter                                         % 以下是论文导言部分，包括论文的封面，中英文摘要和中文目录
\titleformat{\chapter}{\centering\sanhao\song\bfseries}{\chaptername}{}{} % 设置摘要两字的格式
\fancypagestyle{plain}{
\fancyhf{}
\renewcommand{\headrulewidth}{0 pt}
\fancyfoot[C]{\song\xiaowu~\thepage~}
}
\input{preface/cover}                                % 封面


\newif\ifArts       % 添加是否为文科论文的判断（因为文理科要求格式不同
\Artstrue           % 设置文科论文为真。写理科论文时注释这项即可
%%%%%%%%%%   目录   %%%%%%%%%%
\defaultfont
\clearpage{\pagestyle{empty}\cleardoublepage}
\setcounter{page}{1}                                 % 单独从 1 开始编页码
\pagenumbering{arabic}
\titleformat{\chapter}{\centering\sanhao\song\bfseries}{\chaptername}{}{} % 设置目录两字的格式
\pdfbookmark[0]{目~~录}{mulu}
\tableofcontents                                     % 中文目录
\thispagestyle{plain}

\mainmatter\defaultfont\sloppy\raggedbottom
\makeatletter
\fancypagestyle{plain}{                              % 设置开章页眉页脚风格
    \fancyhf{}
    \fancyhead[C]{\song\wuhao \@cheading}            % 首页页眉格式
    \fancyfoot[C]{\song\xiaowu ~\thepage~}           % 首页页脚格式
    \renewcommand{\headrulewidth}{0.5pt}
    \renewcommand{\footrulewidth}{0pt}
}
\makeatother
\setcounter{page}{1}                                 % 单独从 1 开始编页码
\titleformat{\chapter}{\xiaosan\song\bfseries}{\chaptername}{0em}{}
                                                     % 恢复chapter 标题格式要求

\include{body/figures}
\include{body/tables}
\include{body/equations}
\include{body/others}
\include{body/conclusion}
%%%%%%%%%%  参考文献  %%%%%%%%%%
\titleformat{\chapter}{\centering\sanhao\song\bfseries}{\chaptername}{0}{}
\defaultfont
\bibliographystyle{HNUThesis}
\phantomsection
\addcontentsline{toc}{chapter}{参考文献}          % 参考文献加入到中文目录
\nocite{*}                                        % 若将此命令屏蔽掉，则未引用的文献不会出现在文后的参考文献中。
\bibliography{references/reference}

            % 外文资料
\include{appendix/paperInEnglish}              % 外文资料
\include{appendix/paperInChinese}              % 中文译文

\include{appendix/acknowledgements}            % 致谢

\clearpage
% \end{CJK*}                                     % 结束中文字体使用
\end{document}                                 % 结束全文
